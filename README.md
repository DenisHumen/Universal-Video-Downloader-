...
- **Title search** — type a title instead of a link and a dedicated search window opens. Search **all supported services at once** (YouTube, SoundCloud, BiliBili, Niconico — queried in parallel and interleaved) or switch to a single one; browse results with thumbnails, durations and best-available-quality badges, then download in one click or open in the browser.
