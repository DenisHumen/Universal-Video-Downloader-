...
- **Title search** — type a title instead of a link and a dedicated search window opens. Search **all supported services at once** (YouTube, SoundCloud, PornHub and the **YummyAnime** anime catalog — queried in parallel and interleaved) or pick a single service from the scrollable sidebar. Browse results with thumbnails, durations and best-available-quality badges; download videos in one click, open in the browser, or — for anime — open the same episode/voiceover/quality picker you get from a link and grab one episode, a selection or the whole season.
